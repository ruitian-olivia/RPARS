...
<body>
	...
	<div class="about-heading">	
		<div class="container">
			<h2>Resistance Gene Search Result</h2>
		</div>
	</div>
	<!-- gallery -->
	<div class="gallery">
...
if ($_GET['s1'] != ""){
	if ($_GET['r1'] == "NOT" && $_GET['t1'] == "ALL"){
		$qp = sprintf("(`resistance gene ID` NOT LIKE '%%%s%%' OR `gene type` NOT LIKE '%%%s%%' OR `strain name` NOT LIKE '%%%s%%' OR `antibiotic name` NOT LIKE '%%%s%%')", 
		$_GET['s1'], $_GET['s1'], $_GET['s1'], $_GET['s1']);
		$query .= $qp;
	}
	if ($_GET['r1'] == "" && $_GET['t1'] == "ALL"){
		$qp = sprintf("(`resistance gene ID` LIKE '%%%s%%' OR `gene type` LIKE '%%%s%%' OR `strain name` LIKE '%%%s%%' OR `antibiotic name` LIKE '%%%s%%')", 
		$_GET['s1'], $_GET['s1'], $_GET['s1'], $_GET['s1']);
		$query .= $qp;
	}
	if ($_GET['r1'] == "NOT" && $_GET['t1'] != "ALL"){
...
if ($_GET['s2'] != ""){
	if ($_GET['r2'] == "NOT" && $_GET['t2'] == "ALL"){
		$qp = sprintf(" AND (`resistance gene ID` NOT LIKE '%%%s%%' OR `gene type` NOT LIKE '%%%s%%' OR `strain name` NOT LIKE '%%%s%%' OR `antibiotic name` NOT LIKE '%%%s%%')", 
		$_GET['s2'], $_GET['s2'], $_GET['s2'], $_GET['s2']);
		$query .= $qp;
	}
	if ($_GET['r2'] == "OR" && $_GET['t2'] == "ALL"){
		$qp = sprintf(" OR (`resistance gene ID` LIKE '%%%s%%' OR `gene type` LIKE '%%%s%%' OR `strain name` LIKE '%%%s%%' OR `antibiotic name` LIKE '%%%s%%')", 
		$_GET['s2'], $_GET['s2'], $_GET['s2'], $_GET['s2']);
		$query .= $qp;
	}
	if ($_GET['r2'] == "AND" && $_GET['t2'] == "ALL"){
		$qp = sprintf(" AND (`resistance gene ID` LIKE '%%%s%%' OR `gene type` LIKE '%%%s%%' OR `strain name` LIKE '%%%s%%' OR `antibiotic name` LIKE '%%%s%%')", 
		$_GET['s2'], $_GET['s2'], $_GET['s2'], $_GET['s2']);
		$query .= $qp;
	}
	if ($_GET['r2'] == "NOT" && $_GET['t2'] != "ALL"){
